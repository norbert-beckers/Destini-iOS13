...
struct StoryModel {
    ...
    func getChoiceTexts() -> [String] {
        return [stories[storyIndex].choice1.text, stories[storyIndex].choice2.text]
    }
    
    mutating func nextStory(choiceMade userChoice: String) {
        if storyIndex == 0 {
            if stories[storyIndex].choice1.matches(choiceMade: userChoice) {
                storyIndex = stories[storyIndex].choice1.destination
            } else {
                storyIndex = stories[storyIndex].choice2.destination
            }
        }
    }
}
